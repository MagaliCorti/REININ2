...
# EUKARYOTA
EU_Cat <- read.csv("data/euka_cat.csv", header = T, sep = ";")
# order levels of factor
EU_Cat$category <- factor(EU_Cat$category, 
                        levels=c("Animal","Bryophyte","Fungi","Plant", "Protozoa"))
EU_Cat$category <- factor(EU_Cat$category, levels = rev(levels(EU_Cat$category)))
# substitute spaces with underscores
EU_Cat$taxa <- gsub(pattern = "\\ ", replacement="_", 
          x=EU_Cat$taxa )
```
...
Proportion of Fungi, Bryophytes and Vascular Plants that detected from the Eukaryota primer
```{r}
EU_Cat[,-c(1:2)] <- EU_Cat[,-c(1:2)]  %>% mutate_all(as.numeric)

perc_EU <- EU_Cat[,-c(3:42)] %>%
  group_by(category) %>%
  summarise_if(is.numeric, sum)

perc_EU <- perc_EU %>% mutate(Percentage = count/sum(count)*100)
perc_EU
```
In term of reads abundance we can see that Fungi represent the biggest share with 91.6%, followed by Protozoa 3.2%, Animal 2.1%, Plant 1.6%, and Bryophyte 1.5%.
...
# order by Sample_ID
order(sample_meta$Sample_ID, decreasing = F)
sample_meta <- sample_meta[order(sample_meta$Sample_ID, decreasing = F),]

# adding herbivore column to GH matadata dataframe
GH_meta  <- merge(sample_meta, GH, by="Sample_ID")
BR_meta  <- merge(sample_meta, BR, by="Sample_ID")
EU_meta  <- merge(sample_meta, EU, by="Sample_ID")
FU_meta  <- merge(sample_meta, FU, by="Sample_ID")
...
# export sample_meta
write.csv(sample_meta, "outputs/sample_meta.csv", row.names = F)
```
...
EUbar <- merge(EUbar, EU_Cat[,1:2], by="taxa")
...
```{r}
# transpose table of MOTU abundances (GH_tax)
...
GH_taxt <- as.data.frame(apply(GH_taxt, 2, as.numeric), row.names = GH$Sample_ID)
BR_taxt <- as.data.frame(apply(BR_taxt, 2, as.numeric), row.names = GH$Sample_ID)
EU_taxt <- as.data.frame(apply(EU_taxt, 2, as.numeric), row.names = GH$Sample_ID)
FU_taxt <- as.data.frame(apply(FU_taxt, 2, as.numeric), row.names = GH$Sample_ID)
```
...
hEU <- as.data.frame(hEU[12:28,])
hFU <- as.data.frame(hFU[12:39,])

hGH <- as.data.frame(hGH[-c(2,7,10,18),]) 
hBR <- as.data.frame(hBR[-c(2,5,8),]) 
hEU <- as.data.frame(hEU[-c(2,5,8,14),]) 
hFU <- as.data.frame(hFU[-c(2,5,9,17),]) 
...
hEUsplit$herb <- c(
  ...
  rep("Moose", markhEU[3]-markhEU[2]),
  rep("Reindeer", markhEU[4]-markhEU[3]),
  rep("Rodent", nrow(hEUsplit)-markhEU[4]+1))

hFUsplit$herb <- c(
  rep("Hare", markhFU[2]-markhFU[1]),
  rep("Moose", markhFU[3]-markhFU[2]),
  rep("Reindeer", markhFU[4]-markhFU[3]),
  rep("Rodent", nrow(hFUsplit)-markhFU[4]+1))
...
alphaGHdf <- inner_join(alphaGH[,c(1,3,5,6)], as.data.frame(GH_meta[,c(1,3)]), by="Sample_ID")

alphaBRdf <- inner_join(alphaBR[,c(1,3,5,6)], as.data.frame(BR_meta[,c(1,3)]), by="Sample_ID")

alphaEUdf <- inner_join(alphaEU[,c(1,3,5,6)], as.data.frame(EU_meta[,c(1,3)]), by="Sample_ID")

alphaFUdf <- inner_join(alphaFU[,c(1,3,5,6)], as.data.frame(FU_meta[,c(1,3)]), by="Sample_ID")
...
GHdis <- vegdist(GH_taxt)
BRdis <- vegdist(BR_taxt)
EUdis <- vegdist(EU_taxt)
FUdis <- vegdist(FU_taxt) # we have two reindeer samples which only contain 0
...
mod
# we can see that the average distance to median is much lower for Ptarmigan and Moose
...
metaMDS(GHdis) #Stress:     0.1559688  -> should be < 0.1 
metaMDS(BRdis) #Stress:     0.1208233 
metaMDS(EUdis) #Stress:     0.1419643 
